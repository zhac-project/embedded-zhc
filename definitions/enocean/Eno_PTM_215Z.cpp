...
// Tier 2: EnOcean PTM 215Z — self-powered Green Power pushbutton transmitter.
// Pushbutton transmitter module (PTM 215Z / Friends-of-Hue rocker).
// z2m-source: enocean.ts #PTM 215Z (fz.enocean_ptm215z).
//
...
// GPDF `commandID` (16/20/17/21/19/23/18/22/100/101/98/99/34) to the
// action string. embedded-zhc has no Green Power frame family yet:
// `FrameFamily` is only {Zcl, TuyaDp}, `cluster_id_to_name` has no 0x0021,
// and the parser emits no commandNotification frame. Wiring the decoder
// would require a GPDF parse layer (frame-counter dedup + commandID
// extraction) in the core parser — INFRA, out of scope for this vendor
// pass. Schema-only here (from_zigbee/to_zigbee null), exactly as the
...
// z2m e.action([...]) for fz.enocean_ptm215z.
constexpr const char* kActions_PTM_215Z[] = {
...
constexpr Expose kExposes_PTM_215Z[] = {
    {"action", ExposeType::Enum, Access::State, nullptr, nullptr,
     kActions_PTM_215Z, sizeof(kActions_PTM_215Z)/sizeof(kActions_PTM_215Z[0])},
};
...
extern const PreparedDefinition kDef_PTM_215Z{
    ...
    .meta=nullptr, .exposes=kExposes_PTM_215Z, .exposes_count=sizeof(kExposes_PTM_215Z)/sizeof(kExposes_PTM_215Z[0]),
