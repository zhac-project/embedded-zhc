...
// decodes from reports but the generic converter drops — so those were
// dead exposes:
//
...
//
// All three are raw s16 pass-throughs (0.01 °C wire units), mirroring how
...
//
// NB: min/max_cool_setpoint_limit (0x0017/0x0018) are intentionally NOT
// read-decoded here — z2m's fz.thermostat has no cool-limit report branch
// (it reads only abs_min/max_cool at 0x0013/0x0014); z2m populates the
// min/max_cool_setpoint_limit keys optimistically from the tz write path
// only, so the settable exposes remain but are driven by writes/get.
//
