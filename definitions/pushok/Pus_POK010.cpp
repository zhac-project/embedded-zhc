...
// Tier 2: Pushok POK010 — water level + temperature + humidity sensor.
// z2m-source: pushok.ts #POK010. Contact comes from m.binary on
// genBinaryInput(presentValue) — NOT IAS-zone.
// Graduated from generated/: the `contact` channel (the device's headline
// water-level state) was exposed but had NO decoder wired — every contact
// report silently dropped. Wired the vendor kFzPushokContact (genBinaryInput
// presentValue 0x0055 → bool). pulse_counter/operating_mode ride
// genMultistateInput/Value (no generic decoder) → INFRA, exposed only.
...
const FzConverter* const kFz_POK010[] = {
    &kFzPushokContact,   // contact on genBinaryInput(presentValue)
