...
// Tier 3: Aqara DJT11LM vibration/tilt sensor.
//
// Reports vibration events through `ssIasZone` zoneStatus change
// notifications — we can't handle those until the IAS-zone command
// path lands in dispatch (v2). Until then: lumi_basic keeps battery
// visible; the vibration stream is effectively muted.
//
// z2m-source: lumi.ts #DJT11LM (lumi.vibration.aq1).
...
constexpr const char* kModels[] = { "lumi.vibration.aq1" };
}
