...
// Tier 3: Aqara SJCGQ11LM water-leak sensor.
//
// Alarm state arrives as an IAS zone command — the cluster-specific
// command path isn't wired into dispatch yet, so for now only the
// battery TLV on genBasic is exposed. Leak reporting lands with v2's
// IAS handler.
//
// z2m-source: lumi.ts #SJCGQ11LM (lumi.sensor_wleak.aq1).
...
constexpr const char* kModels[] = { "lumi.sensor_wleak.aq1" };
}
