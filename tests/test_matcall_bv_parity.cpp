...
//
// Parity test for the Matcall BV LED dimmer drivers (ZG401224, ZG430700).
// Pins the real gap the auto-generated ports carried against the z2m wire
// shape, plus the FALSE colour suspect.
//
//   * Both use z2m m.light() with no arguments. m.light() defaults
//     powerOnBehavior=true, wiring fz.power_on_behavior (genOnOff 0x4003
//     startUpOnOff enum8) + tz.power_on_behavior + a power_on_behavior enum
//     expose (modernExtend.ts light()). The Tier-1 auto-ports wired only
...
//
//   * FALSE suspect "dropped color/colorTemp": z2m m.light() adds the
//     lightingColorCtrl 0x0300 axis ONLY when called with color/colorTemp
//     args. Both Matcall calls pass neither (plain dimmer drivers), so there
//     is no colour axis to drop. This test asserts NO color_temp / color_x /
//     color_y / hue exposes and no 0x0300 bind so a future colour sweep does
//     not wrongly bolt one on.
//
// z2m's tz.effect (write-only command) has no generic converter and is
// deferred as INFRA — not asserted here.
//
// z2m-source: zigbee-herdsman-converters/src/devices/matcall_bv.ts.
...
extern const PreparedDefinition kDef_ZG401224;  // LED dimmer driver (m.light())
extern const PreparedDefinition kDef_ZG430700;  // LED dimmer driver (m.light())
}  // namespace zhc::devices::matcall_bv
...
void test_dimmers() {
    check_dimmer(devices::matcall_bv::kDef_ZG401224, "ZG401224");
    check_dimmer(devices::matcall_bv::kDef_ZG430700, "ZG430700");
}

}  // namespace

int main() {
    std::printf("test_matcall_bv_parity\n");
    test_dimmers();
    if (g_failures == 0) {
        std::printf("  OK\n");
        return 0;
