...
//
// Parity test for Zipato. One real z2m<->ez gap pinned here after the fix:
//
//   Dropped color/colorTemp axis — rgbw2.zbee27 is z2m
...
//   exposes, bind 0x0300 lightingColorCtrl.
//
// z2m-source: zigbee-herdsman-converters/src/devices/zipato.ts
#include <array>
#include <cassert>
#include <cstdint>
#include <cstring>
...
extern const PreparedDefinition kDef_rgbw2_zbee27;  // RGBW: color + CT
}  // namespace zhc::devices::zipato
...
// ── rgbw2.zbee27: color + colorTemp axes restored. ─────────────────────────
void test_rgbw_light() {
    using namespace zhc::devices::zipato;
    const auto& def = kDef_rgbw2_zbee27;
    ...
    RuntimeContext c3{};
    const std::array<std::uint8_t, 1> hue = {0x7F};
